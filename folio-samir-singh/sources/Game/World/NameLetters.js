...
/**
 * Knockable "SAMIR SINGH" letters + API for the LetterBuilder mascot.
 */
export class NameLetters
{
    ...
    captureAnchors()
    {
        ...
        // Sort along the original name line: Bruno starts at higher X (B) → lower X (N)
    ...
    build(font)
    {
        ...
        const chars = [ 'S', 'A', 'M', 'I', 'R', 'S', 'I', 'N', 'G', 'H' ]
        const wordBreakAfter = 4 // gap after R before second S
        const slots = chars.length + 1 // extra slot for word gap

        // Flip 180° vs Bruno's authored direction so spawn camera reads SAMIR SINGH L→R
        ...
        // Keep letters on the original ground height
    ...
    createLetter(char, font, position, quaternion, index)
    {
        ...
        // Match world palette salmon used by original Bruno letters (#e49a78)
        ...
        // Flip each glyph 180° so the readable face points at spawn camera
        root.rotateY(Math.PI)

        // Lift so the letter sits on the ground (geometry is centered)
